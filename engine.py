...
def train_one_epoch(model: torch.nn.Module, criterion: torch.nn.Module,
    ...
    for samples, targets in metric_logger.log_every(data_loader, print_freq, header):
        ...
        # print(targets['boxes'].shape, targets['labels'].shape)
...
def evaluate(model, criterion, postprocessors, data_loader, base_ds, device, output_dir):
    ...
    for samples, targets in metric_logger.log_every(data_loader, 10, header):
        ...
        outputs = model(samples)
        # print(outputs['pred_boxes'].shape, outputs['pred_logits'].shape)   # torch.Size([1, 100, 4]) torch.Size([1, 100, 21])
        print(targets[0]['boxes'].shape, targets[0]['labels'].shape)            # torch.Size([17, 4]) torch.Size([17])
        # print(targets[0]['boxes'], outputs['pred_boxes'], outputs['pred_logits'].argmax(dim=-1))
        loss_dict = criterion(outputs, targets)
        weight_dict = criterion.weight_dict
